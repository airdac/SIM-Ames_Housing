...
after the imputation, as well as to "test" database. Note that "train" contains 1460 observations of houses that were sold, and "test" contains 1459 different observations of houses we don't know the price of. 

The most relevant conclusions of EDA are:

1 - "Train" and "test" databases contains observations that follows a
similar distribution for all variables, however, some categories
have one more or less levels depending on the database (ex: KitchenAbvGr). However, the probability distribution stays the same.

2 - Distribution of missing values are similar in both databases and are
concentrated in 5 variables: PoolQC, MiscFeature, Alley (+90%), Fence (80%) , FirePlaceQu (50%).

3 - Both databases are hightly unbalanced in almost all categories. This is specially 
relevant in binary variables, where one category retains arround 99%.

4 - Numerical variables have a non normal distribution according to Shapiro–Wilk 
and Kolmogorov-Smirnov tests

5 - The most correlated numerical variables, with more than 50 % of R^2 are: 
GrLivArea, GarageCars, GarageArea, TotalBsmtSF, X1stFlrSF, YearBuilt, FullBath,
YearRemodAdd, GarageYrBlt and TotRmsAbvGrd
